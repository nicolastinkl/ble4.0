...
- (void) connectPeripheral:(CBPeripheral *)peripheral {
    ...
    activePeripheral = peripheral; //id<CBPeripheralDelegate>
...
//        printf("rssi:"+RSSI);
        NSLog(@"rssi : %@",RSSI);
        [[self delegate] TXUploadRSSI:RSSI];
        [[self delegate] TXUploadLog:[NSString stringWithFormat:@"信号强度: %@",RSSI]];
...
- (void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral {
    ...
    [[self delegate] TXUploadLog:[NSString stringWithFormat:@"Connection to peripheral with UUID : %s successfull\r\n",[self UUIDToString:peripheral.UUID]]];
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error {
    if (!error) {
        printf("Characteristics of service with UUID : %s found\r\n",[self CBUUIDToString:service.UUID]);
         [[self delegate] TXUploadLog:[NSString stringWithFormat:@"Characteristics of service with UUID : %s found\r\n",[self CBUUIDToString:service.UUID]]];
        for(int i=0; i < service.characteristics.count; i++) {
            CBCharacteristic *c = [service.characteristics objectAtIndex:i]; 
            ...
            if([self compareCBUUID:service.UUID UUID2:s.UUID]) {
                printf("Finished discovering characteristics");
                 [[self delegate] TXUploadLog:@"Finished discovering characteristics"];
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateNotificationStateForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    if (!error) {
        ...
        [[self delegate] TXUploadLog:[NSString stringWithFormat:@"Updated notification state for characteristic with UUID %s on service with  UUID %s on peripheral with UUID %s ..  \r\n",[self CBUUIDToString:characteristic.UUID],[self CBUUIDToString:characteristic.service.UUID],[self UUIDToString:peripheral.UUID]]];
...
- (void)peripheralDidUpdateRSSI:(CBPeripheral *)peripheral error:(NSError *)error {
    NSLog(@"update rssi: %@", peripheral.RSSI);
    [[self delegate] TXUploadRSSI:peripheral.RSSI];
    //printf("upload rssi %s",peripheral.RSSI);
}
